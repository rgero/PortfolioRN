...
const CourseCard = ({ index, title, languages, short, id }) => {
  ...
  return (
    <Card style={{ width: "100%", marginBottom: 10, backgroundColor: (index % 2 !== 0 ? theme.colors.backdrop : theme.colors.background) }}>
      <Card.Title 
        title={
          <TouchableOpacity onPress={() => navigation.navigate('Course Details', { id })}>
            <Text style={{ color: theme.colors.primary }}>{title}</Text>
          </TouchableOpacity>
        }
        subtitle={languages.join(", ")}
        left={() => <CourseButton id={id} language={languages[0].toLowerCase()} />}
        right={(props) => (
          <IconButton 
            {...props} 
            icon={() => <MaterialIcons name="open-in-browser" size={24} color={theme.colors.primary} />} 
            onPress={() => navigation.navigate('Course Details', { id })}
          />
        )}
      />
